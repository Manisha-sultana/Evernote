    ...
    <div style={{minHeight: '100vh', background: '#f9fafb', color: '#111827'}}>
      <div style={{maxWidth: 1100, margin: '0 auto', padding: 16}}>
        ...
        <div style={{display:'flex',gap:16}}>
          <aside style={{background:'#fff',borderRadius:8,padding:12,width:220}}>
            <h3 style={{fontWeight:600, marginBottom:8}}>Categories</h3>
            <ul style={{listStyle:'none',padding:0,margin:0}}>
              {categories.map((c)=> (
                <li key={c} style={{marginBottom:6}}>
                  <button onClick={()=>{setFilter(c);}} style={{width:'100%',textAlign:'left',padding:8,borderRadius:6,background: filter===c ? '#eef2ff' : 'transparent'}}>{c} {c!=='All Notes' && <span style={{float:'right',color:'#9ca3af'}}>{notes.filter(n=>n.category===c).length}</span>}</button>
                </li>
              ))}
            </ul>
          </aside>
            ...
            {filteredNotes.length === 0 ? (
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:12}}>
                {filteredNotes.map((note) => (
                  <article key={note.id} style={{background:'#fff',padding:12,borderRadius:8,boxShadow:'0 1px 2px rgba(0,0,0,0.05)'}}>
